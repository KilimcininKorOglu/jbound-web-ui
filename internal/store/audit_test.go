...
func TestTheLoginAttemptsAreCountedInsideTheWindow(t *testing.T) {
	f := newFixture(t)
	...
	for i := range 3 {
		...
		if err := attempts.Record(ctx, "192.0.2.1", "dnsadmin", at); err != nil {
			t.Fatalf("cannot record the attempt: %v", err)
		}
	}
	// Another address, which the count must not reach.
	if err := attempts.Record(ctx, "192.0.2.2", "dnsadmin", now); err != nil {
		t.Fatalf("cannot record the attempt: %v", err)
	}
	// An attempt from before the window.
	if err := attempts.Record(ctx, "192.0.2.1", "dnsadmin", now.Add(-time.Hour)); err != nil {
		t.Fatalf("cannot record the attempt: %v", err)
	}

	count, err := attempts.CountSince(ctx, "192.0.2.1", now.Add(-15*time.Minute))
	if err != nil {
		t.Fatalf("cannot count the attempts: %v", err)
	}
	if count != 3 {
		t.Errorf("count = %d, want 3", count)
	}

	if err := attempts.DeleteBefore(ctx, now.Add(-15*time.Minute)); err != nil {
		t.Fatalf("cannot delete the stale attempts: %v", err)
	}

	// The row from an hour ago is gone, and the three inside the window stay.
	var remaining int
	if err := f.db.QueryRow("SELECT COUNT(*) FROM login_attempts").Scan(&remaining); err != nil {
		t.Fatalf("cannot count the table: %v", err)
	}
	if remaining != 4 {
		t.Errorf("%d rows remain, want the four inside the window", remaining)
	}
}
